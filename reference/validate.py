...
# --- 2. partial OT: log form == POT-style multiplicative form ---------------
for s in (0.3, 0.6, 0.9):
    Pl, _, _ = partial_ot_log(C, a, b, eps, s, 3000)
    Pm = entropic_partial_multiplicative(C, a, b, eps, s, 3000)
    ok = np.all(Pl.sum(1) <= a + 1e-6) and np.all(Pl.sum(0) <= b + 1e-6)
    print(f"[partial  ] s={s}  log-vs-mult rel={rel(Pl,Pm):.2e}  mass={Pl.sum():.6f}  P1<=a: {ok}")
...
Pp, _, _ = partial_ot_log(C, a, b, eps, 1.0, 4000)
...
for gamma in (0.3, 0.6, 1.0, 1.5):
    ...
    Pp, _, _ = partial_ot_log(C, a, b, 0.002, Ps.sum(), 40000)
    o1 = (Ps * (C - 2 * gamma)).sum()
    o2 = (Pp * (C - 2 * gamma)).sum()
...
for s in (0.4, 0.75):
    Pf, _, _ = partial_ot_log(Cf, af, bf, 0.05, s, 20000)
